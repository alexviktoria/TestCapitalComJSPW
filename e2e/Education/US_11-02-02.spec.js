...
test.describe("US_11-02-02_Education > Menu item [Shares trading] on UnReg Role", () => {
  ...
  test(`TC_11.02.02_01_UnReg  > Test button [Start Trading] in Main banner on '${language}' language`, async () => {
    ...
    console.log(
      "Testing the first level on the main page is completed successfully",
    );
    await test.step("Checking for links in sidebar items", async () => {
      // await page.waitForTimeout(15000);
      const links = await page.$$eval(
        'a[data-type="sidebar_deeplink"]',
        (elements) => elements.map((el) => el.href),
      );
      if (links.length === 0) {
        console.log(
          "There are no links on this page and testing of the second level is impossible",
        );
      } else {
      ...
      const linksFromFile = fileContent
        .split("\n")
        .filter((link) => link !== "");
      ...
      for (let i = 0; i < randomLinks.length; i++) {
        await test.step("Accidental redirection to the page from the sidebar", async () => {
          ...
          if (links.includes(randomLinks[i])) {
            console.log(
              "Testing on the '${randomLinks[i]}' link was successfully completed",
            );
          } else {
      ...
      for (let i = 0; i < randomLinks.length; i++) {
        await test.step("Accidental redirection to the page from the sidebar", async () => {
          ...
          if (await buttons.SellBtnOnBanner.isVisible()) {
            ...
            console.log(
              `For test on '${randomLinks[i]}' link the button [Sell] doen't displayed`,
            );
            return;
          }
      ...
      for (let i = 0; i < randomLinks.length; i++) {
        await test.step("Accidental redirection to the page from the sidebar", async () => {
          ...
          if (await buttons.BuyBtnOnBanner.isVisible()) {
            ...
            console.log(
              `For test on '${randomLinks[i]}' link the button [Buy] doen't displayed`,
            );
            return;
          }
      ...
      for (let i = 0; i < randomLinks.length; i++) {
        await test.step("Accidental redirection to the page from the sidebar", async () => {
          ...
          if (await buttons.contetntBlockStartTradingBtn.isVisible()) {
            ...
            console.log(
              `For test on '${randomLinks[i]}' link the button [Start Trading] doen't displayed`,
            );
            return;
          }
