...
class AllButtons {
  constructor(page, header) {
    this.page = page;
    this.header = header;
    this.StartTradingBtnOnMainBanner = page.locator(
      'a.btn--darkText[data-type="top_banner_btn"]',
    );
    ...
    this.contetntBlockStartTradingBtn = page.locator(
      '[data-type = "wdg_go_to_market_btn"]',
    );
    // Footer
    this.DownloadOnAppStore = page.locator(
      '[data-type = "banner_capital_ios"]',
    );
    this.DownloadOnGooglePlay = page.locator(
      '[data-type = "banner_capital_google"]',
    );
    this.ExploreWebPlatform = page.locator(
      '[data-type="banner_capital_platform"]',
    );
  ...
  async clickTradeBtnOnWidgetMostTraded() {
    await test.step("Click [Trade] button on Widget 'Most Traded'", async () => {
      try {
        ...
        console.log(
          `For test on FCA license the button [Trade] doen't displayed `,
        );
        test.skip();
      }
    });
    ...
    await test.step("Click [Download on the Google Play] button in the block 'Sign up and trade smart today'", async () => {
      ...
      if (currentUrl === "https://apps.apple.com/IE/app/id1230088754?mt=8") {
        ...
        await test.step("Page of App Store is opened", async () => {
          ...
          await expect(this.ProdactNameOnGoogleplay).toHaveText(
            /Online Broker - Capital.com/,
          );
